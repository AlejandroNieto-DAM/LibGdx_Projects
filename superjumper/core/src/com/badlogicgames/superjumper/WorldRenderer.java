...
public class WorldRenderer {
	...
	private void renderSquirrels () {
		...
		int len = world.planes.size();
		for (int i = 0; i < len; i++) {
			Avion avion = world.planes.get(i);
                        Texture plane = Assets.plane;
			float side = avion.velocity.x < 0 ? 1 : -1;
			if (side < 0)
                            batch.draw(plane, avion.position.x + 0.5f, avion.position.y - 0.5f, side * 1, 1);
			else
                            batch.draw(plane, avion.position.x - 0.5f, avion.position.y - 0.5f, side * 1, 1);
		}
	}
